...
export default function BeachesPage() {
  ...
  useEffect(() => {
    ...
    if (meta)
      meta.setAttribute(
        "content",
        "Eight beaches from La Dolce Vita — from Playa Bonita steps away to Cayo Levantado island and the legendary Playa Rincón. Las Terrenas, Samaná.",
      );
  }, []);
  ...
  return (
    <main className="beaches-page">
      <header className="beaches-header">
        ...
        <h1 className="beaches-title">
          {t("beaches.title")}{" "}
          <span className="beaches-title-gold">{t("beaches.titleGold")}</span>
        </h1>
            ...
            <div className="beach-entry-body">
              <div className="beach-image-wrap">
                {IMAGES[i] ? (
                  ...
                  <div className="beach-image-placeholder">
                    <span className="beach-image-placeholder-name">
                      {beach.name}
                    </span>
                  </div>
                )}
              ...
              <div className="beach-entry-content">
                ...
                <ul className="beach-tags">
                  {beach.tags.map((tag) => (
                    <li className="beach-tag" key={tag}>
                      {tag}
                    </li>
                  ))}
                </ul>
